...
class FullDoc(ElsEntity):
    ...
    # constructors
    def __init__(self, uri = '', sd_pii = '', doi = '', scp_id = ''):
        """Initializes a document given a Scopus document URI or Scopus ID."""
        if uri and not sd_pii and not doi:
            super(FullDoc,self).__init__(uri)
        elif sd_pii and not uri and not doi:
            super(FullDoc,self).__init__(self.__uri_base + 'pii/' + str(sd_pii))
        elif doi and not uri and not sd_pii:
            super(FullDoc,self).__init__(self.__uri_base + 'doi/' + str(doi))
    ...
    def read(self, els_client = None):
        ...
        if super(FullDoc,self).read(self.__payload_type, els_client):
            return True
        else:
            return False
...
class AbsDoc(ElsEntity):
    ...
    def __init__(self, uri = '', scp_id = ''):
        ...
        if uri and not scp_id:
            super(AbsDoc,self).__init__(uri)
        elif scp_id and not uri:
            #super().__init__(self.__uri_base + 'SCOPUS_ID/' + str(scp_id))
             super(AbsDoc,self).__init__(self.__uri_base + 'SCOPUS_ID/' + str(scp_id))
    ...
    def read(self, els_client = None):
        """Reads the JSON representation of the document from ELSAPI.
             Returns True if successful; else, False."""
        if super(AbsDoc,self).read(self.__payload_type, els_client):
            return True
        else:
            return False
