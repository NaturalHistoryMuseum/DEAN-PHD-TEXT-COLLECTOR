...
class ElsSearch():
    ...
    def __init__(self, query, index):
        ...
        self.query = query
        self.index = index
        self._uri = self.__base_url + self.index + '?query=' + self.query + "&count=200"
    ...
    def execute(self, els_client = None, get_all = False, max_results = 100):
        ...
        print("total number of results = ", str(self._tot_num_res))
        self._results = api_response['search-results']['entry']
        if get_all is True:
            while (self.num_res < self.tot_num_res) and (self.num_res < max_results):
                print("results collected",self.num_res)
                ...
                while api_response is None:
                    try:
                        ...
                        print("error waiting 3 seconds and attempting again ... ")
                        time.sleep(3)
                        pass
                self._results += api_response['search-results']['entry']
